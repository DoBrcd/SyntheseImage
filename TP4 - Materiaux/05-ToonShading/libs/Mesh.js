...
class Mesh
{
    ...
    onLoadObj(content, callback=null)
    {
        ...
        for (let l=0; l<lines.length; l++) {
            ...
            if (word0 == 'f' && words.length == 4) {
                ...
                let w1 = words[1].split('/')[0];
                let i1 = filterInt(w1, line);
                let v1 = this.m_VertexList[i1-1];
                // lire le numéro du deuxième point
                let w2 = words[2].split('/')[0];
                let i2 = filterInt(w2, line);
                let v2 = this.m_VertexList[i2-1];
                // lire le numéro du troisième point
                let w3 = words[3].split('/')[0];
                let i3 = filterInt(w3, line);
                let v3 = this.m_VertexList[i3-1];
    ...
    getFacesIndexBufferId()
    {
        ...
        if (this.m_FacesIndexBufferId == null) {
            ...
            if (indexlist.length > 65534) {
                this.m_FacesIndexBufferId = Utils.makeIntVBO(indexlist, gl.ELEMENT_ARRAY_BUFFER, gl.STATIC_DRAW);
                this.m_FacesIndexBufferType = gl.UNSIGNED_INT;
            } else {
                this.m_FacesIndexBufferId = Utils.makeShortVBO(indexlist, gl.ELEMENT_ARRAY_BUFFER, gl.STATIC_DRAW);
                this.m_FacesIndexBufferType = gl.UNSIGNED_SHORT;
            }
        }
    ...
    getEdgesIndexBufferId()
    {
        ...
        if (this.m_EdgesIndexBufferId == null) {
            ...
            // selon le nombre d'indices : entiers 32 bits ou shorts 16 bits
            if (indexlist.length > 65534) {
                this.m_EdgesIndexBufferId = Utils.makeIntVBO(indexlist, gl.ELEMENT_ARRAY_BUFFER, gl.STATIC_DRAW);
                this.m_EdgesIndexBufferType = gl.UNSIGNED_INT;
            } else {
                this.m_EdgesIndexBufferId = Utils.makeShortVBO(indexlist, gl.ELEMENT_ARRAY_BUFFER, gl.STATIC_DRAW);
                this.m_EdgesIndexBufferType = gl.UNSIGNED_SHORT;
            }
        }
    ...
    onDraw(matP, matVM)
    {
        ...
        if (this.m_FacesMaterial != null) {
            ...
            gl.drawElements(gl.TRIANGLES, this.m_TriangleList.length * 3, this.m_FacesIndexBufferType, 0);
        ...
        if (this.m_EdgesMaterial != null) {
            ...
            // dessiner les triangles
            gl.drawElements(gl.LINES, this.m_TriangleList.length * 6, this.m_EdgesIndexBufferType, 0);
...
class Vertex
{
    ...
    computeNormal()
    {
        ...
        for (let triangle of this.m_Mesh.m_TriangleList) {
            if (triangle.contains(this)) {
                // ajouter la normale du triangle courant, elle tient compte de la surface
                vec3.add(this.m_Normal, this.m_Normal, triangle.m_Normal);
            }
        }
...
// fonction qui exige d'avoir un entier correct, elle retourne NaN si ce n'est pas le cas
function filterInt(value, line)
{
  if (/^(\-|\+)?([0-9]+|Infinity)$/.test(value))
    return Number(value);
  throw value+" is not a number in "+line
}
