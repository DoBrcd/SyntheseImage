...
class Utils
{
    ...
    static compileShader(shaderType, source, name)
    {
        ...
        // si on est en WebGL1, alors modifier le source selon son type
        if (! gl.isWebGL2) {
            source = source.replace(/^#version 300 es/g, "#version 100");
            switch (shaderType) {
            ...
            case gl.FRAGMENT_SHADER:
                source = source.replace(/\nin /g, "\nvarying ");
                source = source.replace(/\nout /g, "\n//out ");
                source = source.replace(/texture\(/g, "texture2D(");
                source = source.replace(/glFragColor/g, "gl_FragColor");
                break;
            }
        }
     ...
     * @param values : tableau de GLint à mettre dans le VBO
     * @param vbo_type : type OpenGL du VBO, par exemple gl.ELEMENT_ARRAY_BUFFER
     * @param usage : type de stockage OpenGL des données, par exemple gl.STATIC_DRAW
     * @return identifiant OpenGL du VBO
     */
    static makeIntVBO(values, vbo_type, usage)
    {
        ...
        /*****DEBUG*****/
        if (values.constructor !== Array) {
            error("Utils.makeIntVBO: values vector is empty");
        }
        if (values.length < 1) {
            error("Utils.makeIntVBO: values vector is empty");
        }
        if (vbo_type != gl.ARRAY_BUFFER && vbo_type != gl.ELEMENT_ARRAY_BUFFER) {
            error("Utils.makeIntVBO: second parameter, vbo_type is neither gl.ARRAY_BUFFER ou gl.ELEMENT_ARRAY_BUFFER");
        }
        if (usage != gl.STATIC_DRAW && usage != gl.DYNAMIC_DRAW) {
            error("Utils.makeIntVBO: third parameter, usage is neither gl.STATIC_DRAW or gl.DYNAMIC_DRAW");
        }
        /*****DEBUG*****/
        let buffer = gl.createBuffer();
        gl.bindBuffer(vbo_type, buffer);
    ...
    /**
     * vérifie les indices d'un VBO ELEMENT_ARRAY_BUFFER par rapport à des VBO ARRAY_BUFFER
     * les indices doivent être compris entre 0 et le nombre de sommets des autres VBO - 1
     * les autres VBO doivent avoir le même nombre de sommets
     * de préférence tous les sommets doivent être mentionnés dans les indices
     * cette fonction affiche une erreur ou un avertissement dans la console le cas échéant
     * @param primitive : code de la primitive graphique de dessin : GL.TRIANGLES, GL_LINES, etc.
     * @param indexlist : tableau d'entiers, numéros des sommets
     * @param type : type des données dans les VBOs : VEC2, VEC3 ou VEC4
     * @param autresVBOs : tableaux de sommets
     */
    static checkVBOs(primitive, indexlist, type, ...VBOs)
    {
        // les VBOs doivent faire la même taille
        let vbosize = -1;
        let i = 0;
        for (let vbo of VBOs) {
            i += 1;
            let size = vbo.length;
            if (vbosize < 0) vbosize = size;
            if (vbosize != size) {
                error("Utils.checkVBOs: VBO #"+i+" have not the same size, than VBO #1, "+size+"!="+vbosize);
            }
        }
        // tailles
        let vertexnb = vbosize / type;
        let indexsize = indexlist.length;
        // le nombre de numéros dans le VBO des indices doit correspondre au type de la primitive
        if (primitive == gl.LINES && indexsize % 2 != 0) error("Utils.checkVBOs: bad number of indices to draw gl.LINES, "+indexsize);
        if (primitive == gl.LINE_LOOP && indexsize < 3) error("Utils.checkVBOs: not enough indices to draw gl.LINE_LOOP, "+indexsize);
        if (primitive == gl.TRIANGLES && indexsize % 3 != 0) error("Utils.checkVBOs: bad number of indices to draw gl.TRIANGLES, "+indexsize);
        if (primitive == gl.TRIANGLE_FAN && indexsize < 3) error("Utils.checkVBOs: not enough of indices to draw gl.TRIANGLE_FAN, "+indexsize);
        if (primitive == gl.TRIANGLE_STRIP && indexsize < 3) error("Utils.checkVBOs: not enough of indices to draw gl.TRIANGLE_STRIP, "+indexsize);
        // les numéros dans le VBO des indices ne doivent pas dépasser le nombre de vertices
        i = 0;
        for (let index of indexlist) {
            i += 1;
            //if (index < 0) error("Utils.checkVBOs: index #"+i+" in indexlist is negative: "+index);
            if (index >= vertexnb) error("Utils.checkVBOs: index #"+i+" in indexlist is too big: "+index);
        }
        // est-ce que les sommets sont tous mentionnés ?
        let nums = new Set(indexlist);
        for (let i=0; i<vertexnb; i++) {
            if (! nums.has(i)) console.warn("Utils.checkVBOs: vertex #"+i+" not used in indexlist");
        }
    }
...
function initGL(canvas, options={})
{
    ...
    try {
        ...
        if (gl != null) {
            ...
            gl.isWebGL2 = false;
            console.warning("Pas de WebGL2, seulement WebGL1 !");
        }
