...
public class Couleur
{
    ...
    /**
     * Indique si les deux couleurs sont suffisament proches pour être considéré comme identique
     * @param other
     */
    public boolean IsEquivalentTo(Couleur other) {
    	if(Math.abs(this.r - other.r) >= Constantes.PRECISION_LOW_ANTI_ALIASING) return false;
    	if(Math.abs(this.v - other.v) >= Constantes.PRECISION_LOW_ANTI_ALIASING) return false;
    	if(Math.abs(this.b - other.b) >= Constantes.PRECISION_LOW_ANTI_ALIASING) return false;
    	return true;
    }
