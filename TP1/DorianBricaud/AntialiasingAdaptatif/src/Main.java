...
public class Main extends JPanel implements ComponentListener
{
    ...
    private Couleur getCouleurPixel(final float xe, final float ye, final int maxReflets)
    {
        ...
        if(contact) { 
        	...
        	Couleur colorWithMist = colorObj;
        	
        	return colorWithMist;
        }

        return initial.getSkyColor();
    }
    ...
    public void doTracerImage(final TaskTracerImage task)
    {
        ...
        for (int ye = 0; ye < hauteur; ye++) {
            ...
            for (int xe = 0; xe < largeur; xe++) {
            	//antialiasing progressif
                comtpteurPetitAliasing++;
                Couleur initCouleur = getCouleurPixel(xe+(0.5f), ye+(0.5f), Constantes.MAX_REFLETS);
                ...
                for(int dy = 0; dy < 2; dy++) {
                    ...
                    moyenne = moyenne.div(4);
                }
                if(moyenne.IsEquivalentTo(initCouleur) == false){
                    compteur++;
                    moyenne = new Couleur();
        ...
        System.out.println("nb d'antialiasing plus plus : " + compteur +"\n nb de petit anti aliasing : " + comtpteurPetitAliasing);
