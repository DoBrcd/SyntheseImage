...
public class Sphere extends Objet3D
{
    ...
    {
    	Vecteur N = new Vecteur(centre, incident.contact).normalized();
        ...
        //Coordonnées locales de centre de point le plus proche
        float cercleLon = Math.round(longitude);
        float cercleLat = Math.round(latitude);
        
        //distance entre le centre du cercle et le point de contact
        
        float dist = (float) Math.sqrt((longitude - cercleLon)*(longitude - cercleLon) + (latitude - cercleLat) * (latitude - cercleLat));
        
        boolean inZone = dist <= 0.3f;
        
        if(inZone) {
        	//coord cartésiennes du centre du poids
        	cercleLon = (float) (cercleLon * Math.PI / 9);
        	cercleLat = (float) (cercleLat * Math.PI / 9);
        	
        	float y = (float) Math.sin(cercleLat);
        	float x = (float) (Math.cos(cercleLat) * Math.cos(cercleLon));
        	float z = (float) (Math.cos(cercleLat) * Math.sin(cercleLon));
        	
        	float K = 1.5f;
        	Point centreEncoche = new Point(K * x, K * y, K* z);
        	N = new Vecteur(new Point(N.x, N.y, N.z), centreEncoche).normalized();
        }
