...
public class Sphere extends Objet3D
{
    ...
    public float getIntersectionDistance(final Rayon incident)
    {
        ...
        //verification du point de contact

    	float k = Math.min(k1, k2);
        Point contact = incident.P.add(incident.V.mul(k));
        
        Vecteur N = new Vecteur(centre, contact).normalized();;
        ...
        //PB2 : Couleur en ce point
        int zoneX = (int) Math.floor(longitude * 9 / Math.PI);
        int zoneY = (int) Math.floor(latitude * 9 / Math.PI);
        
        boolean inZone = (zoneX + zoneY) % 2 == 0;
        

        if(inZone) {
        	k = Math.max(k1, k2);
        	if(k != Constantes.INFINI) {
        		contact = incident.P.add(incident.V.mul(k));
	            
	            N = new Vecteur(centre, contact).normalized();
	            //PB1 : Coordonnées sphériques du point de contact
	            longitude = (float) Math.atan2(N.z, N.x);
	            latitude = (float) Math.asin(N.y);
	            
	            
	            
	            //PB2 : Couleur en ce point
	            zoneX = (int) Math.floor(longitude * 9 / Math.PI);
	            zoneY = (int) Math.floor(latitude * 9 / Math.PI);
	            
	            inZone = (zoneX + zoneY) % 2 == 0;
	            if(inZone) {
	            	k = Constantes.INFINI;
	            }
        	}
